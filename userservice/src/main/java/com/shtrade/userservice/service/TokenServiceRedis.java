...
public class TokenServiceRedis implements TokenService {
    ...
    public AuthToken createToken(int userId) {
        ...
        //存储到redis并设置过期时间
        redis.boundValueOps(((Integer)userId).toString()).set(token, constant.getToken_expires_hours(), TimeUnit.HOURS);
        return authToken;
    }
    ...
    public boolean checkToken(AuthToken authToken) {
        ...
        redis.boundValueOps(((Integer)authToken.getUserId()).toString()).expire(constant.getToken_expires_hours(), TimeUnit.HOURS);
        return true;
    }
